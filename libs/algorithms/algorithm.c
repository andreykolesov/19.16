...
bool valueInArea(position max, int i, int j) {
    ...
    return (new_pos.rowIndex <= new_pos.colIndex) && (new_pos.rowIndex <= -new_pos.colIndex);
}
...
long long getSpecialScalarProduct(matrix m) {
    ...
    long long result = getScalarProductRowAndCol(m, max_value.rowIndex, min_value.colIndex);
